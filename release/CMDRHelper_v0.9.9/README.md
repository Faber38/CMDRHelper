...
### Multilingual interface and translation checks

-   the user interface now uses a central multilingual translation system.
-   CMDRHelper now supports **12 interface languages**:
    **German, English, French, Italian, Norwegian (Bokmål), Swedish,
    Finnish, Polish, Dutch, Spanish, Turkish and Greek**.
-   the interface language can be selected and saved in Settings; language
    names are displayed in their own language in the selector.
-   missing translations use a defined fallback order:
    **selected language → English → German → translation key**.
-   translations are maintained centrally in the language files under
    `cmdrhelper/i18n/`.
-   the new developer tool `tools/check_i18n.py` automatically checks:
    -   `tr("...")` keys used by the application,
    -   missing or additional translation keys,
    -   duplicate keys,
    -   mismatching format placeholders such as `{system}` or `{count}`.
-   on Linux the i18n check is automatically executed by `start.sh`.
    Translation problems are shown clearly but do not prevent CMDRHelper
    from starting.
-   mission and Journal processing remains separated from the selected
    CMDRHelper interface language so internal Elite Dangerous data does
    not depend on localized display text.
