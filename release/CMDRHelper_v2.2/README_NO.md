...
**Versjon 2.2** skiller nettjenester, personlige funn, skjermbilder og
leiesoldatkreditter sikkert per kommandør og styrker Journal- og oppdateringsflyten.

### Multi-CMDR-nettjenester

-   Inara- og EDSM-tilgang lagres per kommandør/FID med egen API-nøkkel. Innstillinger
    viser identitet og **konfigurert/ikke konfigurert** og minner om separat nøkkel.
    Valget redigerer bare tilgang: live-opplasting bruker alltid aktiv Journal-FID,
    aldri vist kommandør eller innstillingsvalget.
-   Opplastingsposisjoner og kjørestatuser er separate. Ved FID-bytte blir gamle
    workers ugyldige og kan ikke blokkere eller påvirke den nye kommandøren.
-   Automatisk Inara-overføring bruker en persistent, deduplisert SQLite-outbox,
    batching og retry. Feil blokkerer ikke CMDRHelper; ubekreftede hendelser blir
    hos sin kommandør og prøves bare for aktiv FID. En fremmed outbox lastes aldri
    opp etter bytte.
-   Omfatter FSD-hopp, docking, landinger, Carrier-reiser, posisjon, oppdrag og
    skipskjøp/-salg. Kreditter/assets, cargo, materialer, ShipLocker, loadouts,
    eksobiologi, egen Surface-Mining-historikk og udokumenterte Frontier-hendelser
    hevdes ikke støttet.

### Journal- og delta-sikkerhet

-   Leseren finner først siste komplette Journal-stand. Offset flyttes bare etter
    transaksjonell lokal lagring; Inara-outbox er valgfri. Manglende/ugyldig
    nettoppsett og nettfeil hos Inara/EDSM blokkerer ikke lokale data eller offsets.
-   Delta-/offset-grensetilfeller repareres kontrollert og idempotent, uten
    historisk Inara-masseopplasting.

### Personlig Surface Mining og kartografi

-   `MiningRefined` lagrer faktisk utvunne commodities kumulativt per kommandør,
    system og body. Materialer fra sikker Rhino-kontekst holdes adskilt fra
    `Scan.Materials`. Body-detaljen viser **Egne gruvefunn** og valgfritt
    biproduktmaterialer. **UTVINNING ×N** er fortsatt antall planetsteder; funn
    vises først etter reell utvinning. Et engangs-backfill leser eldre Rhino-
    Journals. CMDRHelper påstår ikke å kjenne depotinnhold på forhånd.
-   `SellExplorationData` og `MultiSellExplorationData` er nå salgsgrensen hos
    Universal Cartographics. Historisk `NavBeaconDetail` regnes ikke som åpent,
    og solgte gamle data kommer ikke tilbake etter rekonstruksjon.

### Leiesoldatkreditter

-   Kommandørens saldo kommer bare fra `Statistics.Bank_Account`, Frontiers navn
    `MercCoins`: aktuell, totalt brukt, Engineering, utstyr og `total_earned`.
    Verdiene er uavhengige, delvise snapshots sletter ikke kjente felt, bare
    entydige sesjoner godtas, og engangs-backfill er idempotent og multi-CMDR-skilt.
-   Frontiers `total_earned` kan avvike matematisk fra saldoen. CMDRHelper beholder
    verdiene uendret og beregner ingen egen balanse eller delta.

### Multi-CMDR-bilder og UI

-   Nye bilder lagres i kommandør+FID-mapper; navn inneholder tid, kommandør og,
    når sikkert, system. Identiteten fryses ved kølegging. Filtre: **Aktuell
    kommandør**, **Alle**, **Ikke tilordnet**. Legacy-bilder endres ikke, og FID
    skiller like navn.
-   Vinduet kan skaleres horisontalt igjen, Explorer-legenden brytes ved behov,
    og nettjenestenes kommandørlister begrenser bredde og popup-posisjon uten at
    lange navn øker minimumsbredden.

### Oppdatering

-   På Windows er worker og omstartet CMDRHelper frakoblet `start.bat`/`cmd.exe`-
    konsollen med ny prosessgruppe, uavhengig konsoll og ingen arvede standard-
    handles. Control-C/Break/interrupt er sikrere; rollback dekker også
    `KeyboardInterrupt` etter endringer, med reparasjonsmarkør ved usikre
    dependencies. To-sekunders handshake består.
-   ZIP lastes asynkront. Reelle bytes driver filnavn, MiB, total, prosent, fart,
    ETA og stolpe; ukjent total bruker busy-modus med mottatte MiB. Trådsikker
    avbrytelse, ufullstendige/ugyldige filer fjernes, og installer starter bare
    etter gyldig ZIP. Synlige faser: nedlasting, ZIP-kontroll, updater-start.
-   Synlig fremdrift for backup, utpakking, kopiering, requirements og omstart er
    ennå ikke del av versjonen.
