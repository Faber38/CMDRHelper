"""Win32 HUD backend: tested with Elite on Windows 11 and with automated tests.
...
def screen_for_monitor(monitor, screens):
    """Match desktop geometry, not Qt's user-presentable screen name.

    Prefer an exact rectangle. With different native/Qt DPI-scaled sizes,
    require a unique largest positive overlap; never guess on a tie.
    """
    geometry = monitor.geometry
    candidates = [(screen, screen.geometry()) for screen in screens]
    matches = [screen for screen, rect in candidates
               if geometry.isValid() and rect == geometry]
    if not matches and geometry.isValid():
        overlaps = []
        for screen, rect in candidates:
            intersection = geometry.intersected(rect)
            if intersection.isValid():
                overlaps.append((screen, intersection.width() * intersection.height()))
        if overlaps:
            largest = max(area for _, area in overlaps)
            matches = [screen for screen, area in overlaps if area == largest]
    if len(matches) != 1:
        reason = "no overlapping Qt screen" if not matches else "multiple equally matching Qt screens"
        raise RuntimeError(
            f"Cannot unambiguously match Elite monitor geometry {geometry.getRect()} "
            f"({monitor.device}): {reason}"
        )
    return matches[0]
...
class WindowsWindowTracker:
    """Output-only window tracking, tested with Elite on Windows 11."""
    ...
    def place_overlay(self, hud, target):
        ...
        screen = screen_for_monitor(monitor, QGuiApplication.screens())
