                ...
                if complete:
                    ...
                    if geo_signals > 0:
                        display_rows.append({
                            "body": "",
                            "entry": {
                                "name": str(row.get("geo_text") or f"GEO ×{geo_signals}"),
                                "scan_type": "geo",
                                "value": 0,
                            },
                            "progress": "–",
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        side.addWidget(self._nav("★  Score", 4))
    ...
    def _explorer_geo_text(body, with_count=True):
        ...
        volcanism = MainWindow._translate_volcanism(
            body.get("volcanism")
        )
    ...
    def _refresh_explorer_live_windows(self):
        ...
        for body in bodies:
            ...
            geo_pending = (
                geo_signals > 0
                and not bool(body.get("self_mapped"))
            )
            ...
            known_value = sum(
                ...
                for entry in popup_species_rows
            )
    ...
    def _score_page(self):
        ...
        header = QHBoxLayout()
        header.addWidget(QLabel("★  System-Score", objectName="sectionTitle"))
        header.addStretch()

        refresh = QPushButton("Neu auswerten", objectName="primary")
        refresh.clicked.connect(self._refresh_score_page)
        header.addWidget(refresh)
        layout.addLayout(header)

        intro = QLabel(
            "Persönliche statistische Auswertung deiner CMDRHelper-Datenbank. "
            "Der Score ist keine Garantie für Funde. Er bewertet nur, welche "
            "Namensmuster in deinen bisherigen Reisen überdurchschnittlich oft "
            "BIO oder wertvolle Körper geliefert haben.",
            objectName="muted",
        )
        intro.setWordWrap(True)
        layout.addWidget(intro)

        check_card, check_layout = self._card("System prüfen")

        input_row = QHBoxLayout()

        self.score_system_edit = QLineEdit()
        self.score_system_edit.setPlaceholderText(
            "z. B. Shrogaae ZL-Z b59-3 oder Flyua Phio NR-C d137"
        )
        self.score_system_edit.returnPressed.connect(self._score_analyse_system)
        input_row.addWidget(self.score_system_edit, 1)

        analyse = QPushButton("Bewerten", objectName="primary")
        analyse.clicked.connect(self._score_analyse_system)
        input_row.addWidget(analyse)

        current = QPushButton("Aktuelles System")
        current.clicked.connect(self._score_use_current_system)
        input_row.addWidget(current)

        check_layout.addLayout(input_row)

        self.score_result_label = QLabel(
            "Noch kein System bewertet.",
            objectName="muted",
        )
        self.score_result_label.setTextFormat(Qt.RichText)
        self.score_result_label.setWordWrap(True)
        check_layout.addWidget(self.score_result_label)

        self.score_evidence_table = QTableWidget(0, 6)
        self.score_evidence_table.setHorizontalHeaderLabels(
            [
                "Grundlage",
                "Muster",
                "Systeme",
        ...
        self.score_evidence_table.setAlternatingRowColors(True)
        self.score_evidence_table.setEditTriggers(QTableWidget.NoEditTriggers)
        self.score_evidence_table.verticalHeader().setVisible(False)
        self.score_evidence_table.horizontalHeader().setSectionResizeMode(
            QHeaderView.Interactive
        )
        self.score_evidence_table.horizontalHeader().setStretchLastSection(True)
        self.score_evidence_table.setMaximumHeight(165)
        self.score_evidence_table.setColumnWidth(0, 110)
        self.score_evidence_table.setColumnWidth(1, 170)
        self.score_evidence_table.setColumnWidth(2, 80)
        self.score_evidence_table.setColumnWidth(3, 80)
        self.score_evidence_table.setColumnWidth(4, 100)
        check_layout.addWidget(self.score_evidence_table)

        layout.addWidget(check_card)

        self.score_tabs = QTabWidget()

        self.score_mass_table = self._score_make_table("Massencode")
        self.score_tabs.addTab(self.score_mass_table, "Massencodes")

        self.score_sector_table = self._score_make_table("Sektor")
        self.score_tabs.addTab(self.score_sector_table, "Beste Sektoren")

        self.score_code_table = self._score_make_table("Code + Masse")
        self.score_tabs.addTab(self.score_code_table, "Code + Massencode")

        layout.addWidget(self.score_tabs, 1)

        self.score_status_label = QLabel("", objectName="muted")
        self.score_status_label.setWordWrap(True)
        layout.addWidget(self.score_status_label)

        QTimer.singleShot(0, self._refresh_score_page)

        return page

    def _score_make_table(self, first_header):
    ...
    def _score_fill_table(self, table, rows, limit=None):
        rows = list(rows or [])
        ...
        for row_index, row in enumerate(rows):
            values = [
                ...
                int(row.get("bio_systems") or 0),
                self._score_percent(row.get("bio_rate")),
                int(row.get("bio_score") or 0),
                int(row.get("value_systems") or 0),
                self._score_percent(row.get("value_rate")),
                int(row.get("value_score") or 0),
                int(row.get("bio_signals") or 0),
            ]
            ...
            for col, value in enumerate(values):
                ...
                if col == 4:
                    item.setForeground(
                        self._score_color(row.get("bio_score"))
                    )
                elif col == 7:
                    item.setForeground(
                        self._score_color(row.get("value_score"))
                    )

                table.setItem(row_index, col, item)

    def _refresh_score_page(self):
        if not hasattr(self, "score_mass_table"):
            return

        try:
            stats = self._score_analyzer().statistics()
        except Exception as exc:
            self.score_status_label.setText(
                f"Score-Auswertung fehlgeschlagen: {exc}"
            )
            return

        self._score_fill_table(
            self.score_mass_table,
            stats.get("mass") or [],
        )

        self._score_fill_table(
            self.score_sector_table,
            stats.get("sectors") or [],
            limit=60,
        )

        self._score_fill_table(
            self.score_code_table,
            stats.get("code_mass") or [],
            limit=80,
        )

        self.score_status_label.setText(
            "Datenbasis: "
            f"{int(stats.get('systems') or 0)} auswertbare prozedurale Systeme · "
            f"BIO-Grundquote {self._score_percent(stats.get('bio_baseline'))} · "
            f"Wert-Grundquote {self._score_percent(stats.get('value_baseline'))}. "
            "Kleine Stichproben werden im Score bewusst abgeschwächt."
        )

    def _score_use_current_system(self):
        current = str(
            getattr(self.state, "system", "") or ""
        ).strip()

        self.score_system_edit.setText(current)
        self._score_analyse_system()

    def _score_analyse_system(self):
        name = self.score_system_edit.text().strip()

        if not name:
            self.score_result_label.setText(
                "Bitte einen Systemnamen eingeben."
            )
            self.score_evidence_table.setRowCount(0)
            return

        try:
            result = self._score_analyzer().score_system(name)
        except Exception as exc:
            self.score_result_label.setText(
                f"Auswertung fehlgeschlagen: {exc}"
            )
            self.score_evidence_table.setRowCount(0)
            return
        ...
        if not result.get("ok"):
            self.score_result_label.setText(
                str(
                    result.get("reason")
                    or "Systemname konnte nicht ausgewertet werden."
                )
            )
            self.score_evidence_table.setRowCount(0)
            return

        bio_score = int(result.get("bio_score") or 0)
        value_score = int(result.get("value_score") or 0)

        bio_color = self._score_color(bio_score).name()
        value_color = self._score_color(value_score).name()

        parsed = result.get("parsed") or {}

        self.score_result_label.setText(
            f"<b>{name}</b><br>"
            f'<span style="color:{bio_color}; font-size:18px; font-weight:700;">'
            f"BIO-Score: {bio_score}/100 – {result.get('bio_level', '')}"
            f"</span>&nbsp;&nbsp;&nbsp;&nbsp;"
            f'<span style="color:{value_color}; font-size:18px; font-weight:700;">'
            f"Explorer-Score: {value_score}/100 – {result.get('value_level', '')}"
            f"</span><br>"
            f"<span style='color:#9aa3ab;'>"
            f"Sektor: {parsed.get('sector', '–')} · "
            f"Code: {parsed.get('code', '–')} · "
            f"Massencode: {parsed.get('mass', '–')}"
            f"</span>"
        )

        evidence = list(result.get("evidence") or [])

        self.score_evidence_table.setRowCount(len(evidence))
        ...
        for row_index, row in enumerate(evidence):
            ...
            for col, value in enumerate(values):
                self.score_evidence_table.setItem(
                    row_index,
                    col,
    ...
    def refresh_all(self):
        ...
        if hasattr(self, "score_mass_table"):
