...
class BodyDetailWindow(QDialog):
    def __init__(self, body: dict, parent=None):
        ...
        self._body_video_widget = None
        self._body_media_player = None
        self._body_audio_output = None
        self._body_audio_button = None
        ...
        # Priorität im Detailfenster:
        # Video -> 2:1-Textur -> normales PNG.
        ...
        if body_image is not None:
            ...
            image_row.addStretch()
            visual_column.addLayout(image_row)

            if self._body_video_widget is not None:
                audio_row = QHBoxLayout()
                audio_row.addStretch()

                self._body_audio_button = QPushButton("🔇 Ton aus")
                self._body_audio_button.setToolTip(
                    "Ton des Videos ein- oder ausschalten"
                )
                self._body_audio_button.clicked.connect(
                    self._toggle_body_video_audio
                )
                audio_row.addWidget(self._body_audio_button)

                audio_row.addStretch()
                visual_column.addLayout(audio_row)

            root.addLayout(visual_column)

            if (
                self._planet_3d_widget is None
                and self._body_video_widget is None
                and self._belt_cluster_widget is None
            ):
    ...
    def _body_video_path(self):
        """Passendes Video zum normalen Körperbild suchen."""
        ...
        for suffix in (".mp4", ".mpg", ".mpeg", ".m4v", ".mov", ".webm", ".avi"):
    ...
    def _body_video_widget_for(self, video_path: Path):
        """Stummes Video im Detailfenster in Endlosschleife."""
        self._body_video_widget = QVideoWidget(self)
        self._body_video_widget.setFixedSize(360, 236)
        self._body_video_widget.setAspectRatioMode(Qt.KeepAspectRatio)
        self._body_video_widget.setToolTip(video_path.name)

        self._body_audio_output = QAudioOutput(self)
        self._body_audio_output.setMuted(True)

        self._body_media_player = QMediaPlayer(self)
        self._body_media_player.setAudioOutput(self._body_audio_output)
        self._body_media_player.setVideoOutput(self._body_video_widget)
        self._body_media_player.mediaStatusChanged.connect(
            self._body_video_status_changed
        )
        self._body_media_player.setSource(
            QUrl.fromLocalFile(str(video_path))
        )
        self._body_media_player.play()

        return self._body_video_widget

    def _body_video_status_changed(self, status):
        if (
            self._body_media_player is not None
            and status == QMediaPlayer.EndOfMedia
        ):
            self._body_media_player.setPosition(0)
            self._body_media_player.play()

    def _toggle_body_video_audio(self):
        if self._body_audio_output is None:
            return

        muted = not self._body_audio_output.isMuted()
        self._body_audio_output.setMuted(muted)

        if self._body_audio_button is not None:
            self._body_audio_button.setText(
                "🔇 Ton aus" if muted else "🔊 Ton an"
            )
    ...
    def _body_visual_widget(self):
        """Video, Cluster, 3D-Kugel oder normales PNG anzeigen."""
        image_name = SystemMapWidget._body_image_name(self.body)

        video_path = self._body_video_path()

        if video_path is not None:
            return self._body_video_widget_for(video_path)
    ...
    def closeEvent(self, event):
        ...
        if self._body_media_player is not None:
            self._body_media_player.stop()
