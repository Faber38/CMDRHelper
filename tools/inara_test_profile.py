"""Standalone INARA profile diagnostic; not part of normal application startup.

Run: python tools/inara_test_profile.py --fid FID
Reads the selected commander's credentials from local CMDRHelper QSettings.
No credentials are embedded in this repository. Sends getCommanderProfile
only; does not change settings or start the application.
"""
...
def main():
    ...
    parser.add_argument("--fid", required=True, help="FID des konfigurierten Commanders")
    args = parser.parse_args()
    fid = args.fid.strip()
    if not fid or "/" in fid or "\\" in fid:
        parser.error("Eine eindeutige Commander-FID ohne Pfadtrenner ist erforderlich.")
    ...
    commander = str(
        settings.value(f"{prefix}/commander", "")
        or ""
    ).strip()

    api_key = str(
        settings.value(f"{prefix}/api_key", "")
        or ""
    ).strip()
