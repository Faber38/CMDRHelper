...
- Stationsbilder, Bildviewer und Details zu bekannten Services, Landeplätzen, Quellen und Zeitstempeln.
- Optionale Spansh-Stationsinformationen unter Einstellungen → Online Services; standardmäßig aus. Nach einem erfolgreichen Abruf sind weitere manuelle Abrufe am selben lokalen Tag gesperrt. Ohne Spansh weist der Stationsreiter auf das begrenzte Journalwissen hin.
- Persönliche Schiffs- und Carrierbilder, Typ-/Standardbilder und Bildviewer. Schiffe lassen sich aus der Flotte entfernen und über „Alle Schiffe neu einlesen“ wiederherstellen; verkaufte Schiffe werden zuverlässiger erkannt.
- Verbesserte Systemkarte, Wertliste mit farbigem Kartierungsstatus, Cargo-HUD und Navigation. Weniger Hintergrundlast unter Linux sowie bei Screenshots, Galerie und Explorer.
- Behoben: Harmlose kurze Elite-Journale unterbrechen die Mining-Carrierbestände nicht mehr. Unbekannte oder unsichere Daten bleiben weiterhin als solche erkennbar.
...
- Reaktionsfähigere Oberfläche während langer Datenverarbeitung.
- Verbesserte Nachholung neuer und gewachsener Journale nach dem Import.
- Schnellere Archivverarbeitung und Mining-Aktualisierung.
- Systemnamen in der geöffneten Chronik-Systemansicht per Klick kopieren.
- Darstellung der Tabs unter Windows im Dark Mode korrigiert.

## Neu in Version 3.4.5
...
- Kartierungen bleiben auch erhalten, wenn der zugehörige Scan erst später im aktuellen Journal erscheint.
- Eigener Kartierungsstatus, Kartierungszeit und Sondeneffizienz werden zuverlässig gespeichert.
- Der Archivimport ersetzt vorhandene Scan- und Kartographiewerte nicht mehr durch ungültige Nullwerte.
- Bereits betroffene Körper mit vollständigen Körperdaten erhalten beim Laden oder Aktualisieren wieder korrekte Anzeigewerte.
- Kartographieschätzungen für Live berücksichtigen jetzt die korrekten Boni; Credit-Nachkommastellen werden erst am Ende abgeschnitten.
- Aus Verkäufen gelernte Korrekturfaktoren verändern die regulären Explorationswerte nicht mehr.
- Zusätzliche Regressionstests sichern sitzungsübergreifende Kartierungen und die Speicherung der Kartierungsdaten ab.
