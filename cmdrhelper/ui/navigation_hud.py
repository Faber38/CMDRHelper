...
class NavigationHud(QWidget):
    ...
    def set_enabled(self, enabled):
        self._windows_failed = False
        if enabled:
            # WindowTransparentForInput must install an empty X11 ShapeInput.
            # Fail closed before mapping if the platform does not honor it.
            wid = int(self.winId())
            QGuiApplication.sync()
            try:
                self.safe_input = self.tracker.input_is_empty(wid)
                if self._native_windows and not self.safe_input:
                    raise RuntimeError("Windows HUD input styles are unsafe")
            ...
            if not self.safe_input:
                ...
                self.timer.stop()
                self.hide()
                self._status("error", "HUD input region is not empty")
    ...
    def follow_target(self):
        ...
        if not self.enabled or not hud_lines(self.controller.state):
            self.hide()
            self._status("waiting_navigation" if self.enabled else "off")
            return
        try:
            target = self.tracker.current() if self.enabled and self.safe_input else None
        except (OSError, RuntimeError, ValueError) as exc:
            if not self._native_windows:
                raise
            self._windows_failure(exc)
        ...
        if target is None:
            ...
            reason = getattr(self.tracker, "reason", "not_found") if self.enabled else "off"
    ...
    def paintEvent(self, event):
        ...
        top = 14.
        for text, pixel_size, gap in zip(hud_lines(self.controller.state),
                                         (38, 26, 18), (10, 8, 8)):
            font = painter.font()
            font.setBold(False)
