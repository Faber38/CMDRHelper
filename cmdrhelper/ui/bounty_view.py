...
class BountyView(QFrame):
    ...
    def reset_dialog(self):
        dialog = QMessageBox(QMessageBox.Question, tr("bounties.reset_title"),
                             tr("bounties.reset_text"), parent=self)
    ...
    def confirm_reset(self):
        ...
        if dialog.clickedButton() is confirm:
            if not self.manager.manual_reset(fid):
                QMessageBox.warning(self, tr("bounties.reset_title"), tr("bounties.reset_failed"))
        dialog.deleteLater()

    def refresh(self):
        data = self.manager.snapshot()
        self.title.setText(tr("bounties.title"))
        self.reset_button.setText(tr("chronicle.reset") + "…")
        self.reset_button.setEnabled(bool(self.manager.fid))
        self.total.setText(f'{tr("mining.total")}: {self.format_reward(data["total"])}')
        self.setToolTip(tr("bounties.hint"))
        self.empty.setText(tr("bounties.empty"))
        ...
        self._resize_table()
        self.empty.setVisible(not rows and not data["uncertain"] and not self.manager.storage_error)
        notice = tr("bounties.hint")
        if data["from_now"]:
            notice += "\n" + tr("bounties.from_now")
        if data["uncertain"]:
            notice += "\n" + tr("bounties.uncertain")
        if self.manager.storage_error:
            notice += "\n" + tr("bounties.storage_error")
