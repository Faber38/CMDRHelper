...
<h3>Odyssey</h3>
<p>“Stolen” marks an inventory stack explicitly recorded as stolen in the saved Elite data. A missing flag does not confirm that it is not stolen. If an owner identifier is known, the tooltip shows it as “Owner”; an unknown owner name is not filled in.</p>
<p>The “Current availability unconfirmed” tooltip indicates an unconfirmed source for the relevant special materials. CMDRHelper cannot provide confirmed current acquisition information; this does not mean the item is unobtainable.</p>
...
<p><b>Origin</b><br>Surface means planetary surface mining; Asteroid means asteroid/ring mining. Some commodities come from both sources (Both) and appear in both matching origin filters.</p>""",
              ...
              '<p>Use “Missions →” to open the “Missions &amp; Rewards” main section with the '
              'known mission objectives and status information.</p>\n'
              '\n'
              '<h3>Latest Status</h3>\n'
              '<p>“Latest Status” summarizes the last known persistent commander state. This allows '
              'important information to be restored even after restarting Elite Dangerous or '
              'CMDRHelper.</p>\n'
              '\n'
              '<h3>Recently visited systems</h3>\n'
...
<p>Defaults use the active commander and ship. Merely viewing another commander in CMDR view does not change that basis.</p>"""),
 'images': ('Pictures',
            ...
            '\n'
            '<h3>Save settings</h3>\n<p>“Save settings” saves source and destination folders, output format, brightening and both checkbox states. Monitoring is set up again using these choices; existing BMP files are marked as known. The gallery is also refreshed.</p>\n<p>“Refresh gallery” reloads the gallery from existing image files for the current gallery filter. It does not start BMP conversion.</p>\n\n<h3>Automatic processing</h3>\n'
            '<p>If “Automatically convert new BMP files” is activated and valid source and destination folders '
            ...
            'manner.</p>\n'
            '<p>By default, the original BMP file is retained. If “Delete BMP after successful conversion” is '
            ...
            '<p>It will be in the subfolder <b>UNKNOWN_&#8203;UNKNOWN/</b> processed; the file name also '
...
<p>Saved information can be viewed offline and after a restart; it represents the last known state. Images, deletion and rereading affect CMDRHelper only. They do not change ships, carriers or credits in Elite Dangerous or rewrite journals.</p>""",
 ...
 'settings': ('Settings',
              ...
              'settings are managed separately by Frontier ID if necessary.</p>\n\n<h3>Quick favorite</h3>\n<p>Under “Quick favorite”, use “Set hotkey” to assign a global shortcut or “Change hotkey” to change it. “Remove hotkey” removes the assignment; the initial state is “Not assigned”. The choice is saved. A registration conflict produces a message. The shortcut saves a valid current surface position as a commander-specific favourite without a dialog, not a screenshot. Nothing is saved without suitable position data.</p>\n'
              '\n'
              '<h3>journal</h3>\n'
              ...
              '<p>Manual refresh can bypass the 7-day freshness period and a failed automatic attempt that day. If the system was already retrieved successfully today by the local calendar, no new request is made: “Spansh data has already been updated today.” A successful retrieval renews the station cache. On failure, existing local and usable cached information remains available and the status line reports the failure. A failed manual attempt can be retried.</p>\n\n<h3>EDSM body data and cache</h3>\n<p>“Use EDSM” also controls supplementary body data for the active journal commander’s current system in Explorer. After “Save online access” and during normal journal updates, a usable cache is loaded or EDSM is queried in the background. The public body lookup requires a network connection but no API key; journal uploads are a separate operation.</p>\n<p>Successfully fetched body data is stored locally per system and can be reused from the cache for up to 24 hours, including after restarts. Older data prompts a new request on a suitable update. When disabled, body data from this cache is not added to the display and no new body requests start; cache files are not deleted. Local journal data remains usable. Request failures do not produce invented bodies.</p>\n'
              '\n'
              ...
              '<p>The sidebar switches under “auto show” work independently: “Valuable bodies”, “BIO findings”, “GEO” and “Cargo hold” control their respective live windows. “Cargo HUD” displays cargo in the overlay, while “Navigation HUD” displays active planetary navigation. These display switches do not themselves start online queries; HUDs require Elite in the foreground and suitable data.</p>\n<p>“EDSM status HUD” instead needs its own public EDSM network query, independent of “Use EDSM”, its body cache and an API key. Inara uploads and Spansh station information have their own service switches; HUD switches do not enable them.</p>\n\n<h3>Updates</h3>\n'
