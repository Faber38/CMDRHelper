...
<h3>Odyssey</h3>
<p>„Skradzione” oznacza stos wyraźnie zapisany jako skradziony w danych Elite. Brak oznaczenia nie potwierdza, że stos nie jest skradziony. Znany identyfikator właściciela pojawia się w podpowiedzi jako „Właściciel”; nieznana nazwa właściciela nie jest dopisywana.</p>
<p>Podpowiedź „Aktualna dostępność niepotwierdzona” wskazuje niepotwierdzone źródło pozyskania odpowiednich materiałów specjalnych. CMDRHelper nie ma potwierdzonej aktualnej informacji o ich pozyskaniu; nie oznacza to, że przedmiotu nie można zdobyć.</p>

<p>Czwarta karta w sekcji Materiały zawiera Towary, Materiały, Dane i Materiały zużywalne. Schowek i plecak pokazują zapasy osobiste. Lotniskowiec ✎ pokazuje ręcznie potwierdzony prywatny zapas towarów, materiałów i danych na własnym lotniskowcu. Kliknij dwukrotnie, aby potwierdzić, poprawić lub ustawić jako nieznany. — oznacza nieznany; 0 wymaga wyraźnego potwierdzenia. Kolumna „Łącznie” obejmuje schowek, plecak i lotniskowiec tylko przy znanych i spójnych wartościach. Przy wielu stosach zapas lotniskowca pojawia się raz w podsumowaniu; stosy pozostają oddzielne. Materiały zużywalne zachowują sumę osobistą bez lotniskowca. FCMaterials nie jest pełnym spisem zapasów lotniskowca i nie jest tak używany. Limit schowka wynoszący 1000 dotyczy kategorii, nie pojedynczego przedmiotu. Zapas lotniskowca jest obliczany od ostatniego potwierdzenia. Zmiany schowka są księgowane przeciwnie tylko na własnym lotniskowcu, po odjęciu jawnych transakcji osobistych. Zakupy i sprzedaż u barmana, zwłaszcza innych graczy, mogą zmienić rzeczywisty zapas bez automatycznej rejestracji. W razie potrzeby potwierdź ponownie dwukrotnym kliknięciem. Suma używa prognozy od ostatnio potwierdzonego zapasu, bez gwarancji odczytu na żywo. Prywatny magazyn Odyssey dzieli 1 000 miejsc między towary, materiały i dane. Suma zapasów jest kompletna dopiero po potwierdzeniu wszystkich pozycji i dodatkowych materiałów, także zer. Inaczej pokazywane jest minimum. Po przekroczeniu limitu wpisy pozostają, a Suma staje się nieznana. Szafka, plecak i rezerwacje rynkowe nie są prywatnym zapasem lotniskowca.<br><b>! – Konfiguracja zapasu lotniskowca</b><br>Kliknij dwukrotnie kolumnę Lotniskowiec i wpisz aktualną ilość dla KAŻDEJ pozycji. Potwierdź również WSZYSTKIE puste pozycje wartością 0.<br>— = jeszcze niepotwierdzone / nieznane<br>0 = wyraźnie potwierdzony pusty zapas</p>
<p>Otwarte zlecenia kupna u barmana rezerwują miejsce. Zajętość w grze może przekraczać zapas materiałów. Rezerwacje nie są materiałami i nie wchodzą do ich sum. Bez odpowiednio aktualnych danych rynkowych zajętość pozostaje nieznana. Transakcje innych graczy mogą zmienić ten stan.</p>
...
<p>Osobisty ekwipunek jest automatycznie aktualizowany w tle. Potwierdzone nowe zebrania mogą być krótko wyróżniane. Przy zmianie dowódcy stare stany są natychmiast usuwane. Podkarty, filtry, szerokości i kolejność kolumn są zapisywane oddzielnie dla Odyssey.</p>
<h3>Mining</h3>
...
<p><b>Pochodzenie</b><br>Surface oznacza wydobycie na powierzchni planet; Asteroid oznacza wydobycie w asteroidach/pierścieniach. Niektóre surowce pochodzą z obu środowisk (Both) i pojawiają się w obu odpowiednich filtrach pochodzenia.</p>""",
              ...
              '<p>„Misje →” otwiera główny dział „Misje i nagrody” ze '
              'znanymi celami misji i informacjami o ich stanie.</p>\n'
              '\n'
              '<h3>Ostatni stan</h3>\n'
              ...
              'CMDRHelper.</p>\n'
              '\n'
              '<h3>Ostatnio odwiedzone systemy</h3>\n'
...
<p>Domyślne dane pochodzą od aktywnego dowódcy i jego statku. Samo oglądanie innego dowódcy w widoku CMDR tego nie zmienia.</p>"""),
 'images': ('Kino',
            ...
            '\n'
            '<h3>Zapisz ustawienia</h3>\n<p>„Zapisz ustawienia” zapisuje foldery źródłowy i docelowy, format, rozjaśnienie oraz oba pola wyboru. Monitorowanie jest ponownie ustawiane według tych opcji; istniejące BMP zostają oznaczone jako znane. Galeria również jest odświeżana.</p>\n<p>„Odśwież galerię” ponownie wczytuje galerię z istniejących plików obrazów dla bieżącego filtra. Nie uruchamia konwersji BMP.</p>\n\n<h3>Automatyczne przetwarzanie</h3>\n'
            '<p>Jeśli włączona jest opcja „Automatycznie konwertuj nowe pliki BMP” i ustawione są prawidłowe '
            ...
            'BMP po udanej konwersji”, źródłowy BMP zostanie usunięty dopiero po pomyślnym zapisaniu '
            'obrazu docelowego.</p>\n'
            '\n'
            ...
            '<p>Będzie w podfolderze <b>UNKNOWN_&#8203;UNKNOWN/</b> obrobiony; nazwa pliku używana '
            'również w przypadku Commandera <b>UNKNOWN</b>. Folder ten można przeglądać poprzez '
...
<p>Zapisane informacje można oglądać bez sieci i po ponownym uruchomieniu; przedstawiają ostatni znany stan. Obrazy, usuwanie i ponowny odczyt dotyczą wyłącznie CMDRHelper. Nie zmieniają statków, lotniskowców ani kredytów w Elite Dangerous i nie przepisują dzienników.</p>""",
 ...
 'settings': ('Ustawienia',
              ...
              'Frontier.</p>\n\n<h3>Szybki ulubiony</h3>\n<p>W „Szybki ulubiony” wybierz „Ustaw skrót”, aby ustawić globalny skrót, lub „Zmień skrót”, aby go zmienić. „Usuń skrót” usuwa przypisanie; początkowo widnieje „Nie przypisano”. Wybór jest zapisywany. Konflikt rejestracji wyświetla komunikat. Skrót zapisuje prawidłową bieżącą pozycję na powierzchni jako ulubione dowódcy bez okna dialogowego, nie zrzut ekranu. Bez odpowiednich danych pozycji nic nie zostaje zapisane.</p>\n'
              '\n'
              '<h3>dziennik</h3>\n'
              '<p>Folder dziennika jest jednym z najważniejszych ustawień. Musi wskazywać folder, '
              'w którym znajduje się plik Elite Dangerous <code>Journal*.log</code> pliki używanego '
              ...
              '<p>Aktualizacja ręczna może ominąć okres 7 dni i nieudaną automatyczną próbę danego dnia. Jeśli system został już dzisiaj pomyślnie pobrany według lokalnego kalendarza, nowego zapytania nie będzie: „Dane Spansh zostały już dzisiaj zaktualizowane.” Udane pobranie odnawia pamięć podręczną stacji. Przy błędach pozostają dane lokalne i użyteczne dane podręczne, a wiersz stanu pokazuje niepowodzenie. Nieudaną próbę ręczną można ponowić.</p>\n\n<h3>Dane ciał EDSM i pamięć podręczna</h3>\n<p>„Używaj EDSM” steruje też dodatkowymi danymi ciał bieżącego systemu aktywnego dowódcy dziennika w Explorerze. Po „Zapisz dostęp online” i podczas zwykłej aktualizacji dziennika wczytywana jest użyteczna pamięć podręczna albo EDSM jest odpytywane w tle. To publiczne pobieranie wymaga sieci, lecz nie klucza API; wysyłanie dziennika jest osobną operacją.</p>\n<p>Pobrane dane ciał są zapisywane lokalnie według systemu i mogą być używane z pamięci podręcznej przez maksymalnie 24 godziny, także po ponownym uruchomieniu. Starsze dane powodują nowe zapytanie przy odpowiedniej aktualizacji. Po wyłączeniu funkcji pamięć ta nie uzupełnia widoku i nie rozpoczynają się nowe zapytania o ciała; pliki pamięci nie są usuwane. Lokalne dane dziennika pozostają dostępne. Błąd pobrania nie tworzy fikcyjnych ciał.</p>\n'
              '\n'
              ...
              '<p>Przełączniki „pokazuj automatycznie” na pasku bocznym działają niezależnie: „Wartościowe ciała”, „Znaleziska BIO”, „GEO” i „Ładownia” sterują odpowiednimi oknami na żywo. „HUD ładowni” pokazuje ładunek w nakładce, a „HUD nawigacji” aktywną nawigację planetarną. Te przełączniki wyświetlania same nie inicjują zapytań sieciowych; HUD wymaga Elite na pierwszym planie i odpowiednich danych.</p>\n<p>„HUD statusu EDSM” wymaga natomiast własnego publicznego zapytania do EDSM, niezależnie od „Używaj EDSM”, pamięci ciał i klucza API. Wysyłanie do Inara oraz informacje stacji Spansh mają własne przełączniki usług; przełączniki HUD ich nie włączają.</p>\n\n<h3>Aktualizacje</h3>\n'
