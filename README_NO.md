...
- Rettet feil med svært store oppdrags-ID-er, slik at oppdrag lagres pålitelig.
- Etter en lagringsfeil prøver journalbehandlingen på nytt med stadig lengre pauser, uten å hoppe over oppføringer.
- Forbedret oppstart når ingen aktiv kommandør er registrert ennå.
- Mer nyttig teknisk diagnostikk for disse feilene, uten å logge konkrete oppdrags-ID-er.
...
- Bedre visning av planeter, måner og flerstjernesystemer
- Trygg databaseoppdatering med automatisk sikkerhetskopi og tilbakerulling ved feil
- Elites journalfiler forblir uendret
- Flere stabilitets- og visningsforbedringer

Databaseoppdateringen retter eldre lagrede forhold mellom stjerner, planeter og måner. Journalene blir bare lest. Avslutt Elite Dangerous først og gjør historiske journaler tilgjengelige hvis mulig. Hele CMDRHelper-databasen sikkerhetskopieres på forhånd; ved feil rulles endringene tilbake, og sikkerhetskopien gjenopprettes om nødvendig. Kopien beholdes for sikkerhets skyld. Avbryt lar deg utsette oppdateringen.

## Nytt i versjon 3.4 – Mining og beholdning

- Ny Mining-del med varer, referansepriser og verdiklasser.
- Skip-/SRV-beholdning og manuelt bekreftet carrierbeholdning med automatisk sporing av overføringer.
- Kombinerbart søk og filtre for planetutvinning og asteroider/ringer.
- ABBAU ×N åpner riktig Mining-oversikt direkte.
- Korrekte kartografiverdier ved øktbytte.
- Forbedret hjelp, tabellutseende og betjening.

Én felles tabell inneholder 57 gruvevarer med opprinnelse Surface, Asteroid eller Both. Kjente referansepriser og avledede verdiklasser er faste veiledende verdier, ikke direkte markedsdata.

Verifisert skip-/SRV-last; dobbeltklikk for å bekrefte, korrigere eller tilbakestille carrierbeholdningen til ukjent. Bare entydige CargoTransfer-hendelser til/fra din egen carrier følges fra en bekreftet startbeholdning. Total vises bare når begge mengdene er kjent; ukjent forblir —.

Filtre for positiv beholdning, verdiklasse og opprinnelse kan kombineres; tilstandene, sortering og kolonnebredder lagres. ABBAU ×N velger automatisk planetutvinning. Manuell lastoppdatering har statusanimasjon og bevarer carrierverdier.

Forbedret Mining-tabell, carrierverktøytips og versjonsuavhengig integrert Mining-hjelp. Håndpeker på eksisterende interaktive himmellegemer og kontroller i systemkartet. A-6-a-plassholderrettingen bevarer korrekte kartografiverdier mellom økter.

## Nytt i versjon 3.3.1 – Feilrettinger

- Fullført DSS-kartlegging registreres nå pålitelig direkte.
- Kartleggingen beholdes selv om den tilhørende skanningen vises senere i den aktuelle journalen.
- Din kartleggingsstatus, kartleggingstid og sondeeffektivitet lagres pålitelig.
- Arkivimport erstatter ikke lenger eksisterende skanne- og kartleggingsverdier med ugyldige nullverdier.
- Berørte legemer med fullstendige data får riktige visningsverdier igjen ved innlasting eller oppdatering.
- Kartleggingsanslag for Live inkluderer nå riktige bonuser; desimaler i kredittbeløp kuttes først til slutt.
- Korreksjonsfaktorer lært fra salg endrer ikke lenger vanlige utforskningsverdier.
- Flere regresjonstester dekker kartlegging på tvers av økter og varig lagring av kartleggingsdata.

## Nytt i versjon 3.3

- Ny systemanalyse basert på din personlige utforskningserfaring.
- Spillmodus Open / Solo / Privat gruppe direkte i oversikten.
- Kopier systemnavn fra siste systemer med ett klikk.
- Tydeligere erfaringsdata og mer forståelig analyse.
- Mer pålitelig import av journalarkiv når journalfiler utvides senere.

Det tidligere hopptipset heter nå Analyse, med Systemanalyse og de eksisterende erfaringsdataene. Systemanalysen sammenligner et mål med din personlige historikk: Massekoden gir grunnestimatet, mens region og familie finjusterer det forsiktig. Potensialindeks 100 tilsvarer ditt personlige historiske gjennomsnitt av dempet utforskningspotensial, ikke en prosentvis sannsynlighet. Datagrunnlag og pålitelighet holdes atskilt fra vurderingen; sluttnummeret påvirker ikke poengsummen, og BIO er foreløpig bare informasjon.
